 ...
 * KORRIGIERTE VERSION: Nutzt standardisierte Fehlerbehandlung für besseres Debugging.
 */
function csv_import_validate_handler() {
    ...
    try {
        ...
        if ( $type === 'config' ) {
            ...
            $response_data = csv_import_validate_csv_source( $type, $config );
        }

        csv_import_log( 'debug', "Validierung durchgeführt: {$type}", ['valid' => $response_data['valid'] ?? false] );

        if ( !empty($response_data['valid']) ) {
            wp_send_json_success( $response_data );
        } else {
            // Stellt sicher, dass immer eine Nachricht gesendet wird
            if (empty($response_data['message'])) {
                $response_data['message'] = 'Die Validierung ist fehlgeschlagen, aber es wurde keine spezifische Fehlermeldung zurückgegeben.';
            }
