...
class CSV_Import_Pro_Admin {
    ...
    private function render_page($template_file) {
        $data = [];
        if (function_exists('csv_import_get_progress')) { $data['progress'] = csv_import_get_progress(); }
        if (function_exists('csv_import_get_config')) { $config = csv_import_get_config(); $data['config'] = $config; if(function_exists('csv_import_validate_config')) { $data['config_valid'] = csv_import_validate_config($config); } }
